...
void worker_thread_process(WorkerThreadData *data) {
    ...
    volatile uint32_t *colorReducedPixelOut =
            data->lastColorReducedPixels + rowOffset * data->width;
                ...
                if (0 != (*pixels >> 24)) {
                    ...
                    for (int directionId = 0;
                        ...
                        volatile uint32_t *pixel =
                                pixels + ERROR_PROPAGATION_DIRECTION_X[directionId] +
                                ERROR_PROPAGATION_DIRECTION_Y[directionId] * data->width;
                        if (x + ERROR_PROPAGATION_DIRECTION_X[directionId] >= data->width ||
                            y + ERROR_PROPAGATION_DIRECTION_Y[directionId] >= ditherRowCount ||
                            0 == (*pixels >> 24)) {
                            continue;
                        }
                    ...
                    *colorReducedPixelOut = (0xFF000000 | (cube->color[BLUE] << 16) |
                    ...
                    if (data->useDither) {
                        ...
                        for (int directionId = 0;
                            ...
                            int32_t weight = ERROR_PROPAGATION_DIRECTION_WEIGHT[directionId];
                            int32_t dstR = ((int32_t) ((*pixel) & 0xFF) +
                                            (diffR * weight + 8) / 16);
                            int32_t dstG = (((int32_t) ((*pixel) >> 8) & 0xFF) +
                                            (diffG * weight + 8) / 16);
                            int32_t dstB = (((int32_t) ((*pixel) >> 16) & 0xFF) +
...
bool FastGifEncoder::init(uint16_t width, uint16_t height, const char *fileName) {
    ...
    for (int i = 0; i < threadCount - 1; i++) {
        ...
        pthread_create(workerThreadData[i].workerThread, NULL, worker_thread,
                       &(workerThreadData[i]));
    }
...
bool FastGifEncoder::writeContents(Cube *cubes, uint8_t *pixels, uint16_t delay,
                                   const EncodeRect &encodingRect) {
    writeNetscapeExt();
...
bool FastGifEncoder::writeNetscapeExt() {
    ...
    const uint8_t netscapeExt[] = {0x21, 0xFF, 0x0B, 'N', 'E', 'T', 'S', 'C', 'A', 'P', 'E', '2',
                                   '.', '0', 0x03, 0x01, 0x00, 0x00, 0x00};
...
bool FastGifEncoder::writeGraphicControlExt(uint16_t delay) {
    ...
    const uint8_t graphicControlExt[] = {0x21, 0xF9, 0x04, packed, (uint8_t) (delay & 0xFF),
                                         (uint8_t) (delay >> 8), 0xFF, 0x00};
...
bool FastGifEncoder::writeFrame(Cube *cubes, uint8_t *pixels, const EncodeRect &encodingRect) {
    ...
    uint8_t packed = (localColorTableFlag << 7) | (interlaceFlag << 6) | (sortFlag << 5) |
...
bool FastGifEncoder::writeBitmapData(uint8_t *pixels, const EncodeRect &encodingRect) {
    uint8_t *endPixels =
            pixels + (encodingRect.y + encodingRect.height - 1) * width + encodingRect.x +
            encodingRect.width;
    uint8_t dataSize = 8;
            ...
            for (uint32_t x = 0; x < width; ++x) {
                if (0 != (*ditherPixels >> 24)) {
                    ...
                    for (int directionId = 0;
                         directionId < ERROR_PROPAGATION_DIRECTION_NUM; ++directionId) {
                        uint32_t *pixel =
                                ditherPixels + ERROR_PROPAGATION_DIRECTION_X[directionId] +
                        ...
                        *pixel = (dstA << 24) | (newB << 16) | (newG << 8) | newR;
                        ...
                        uint8_t *pixelOut =
                                ditherPixelOut + ERROR_PROPAGATION_DIRECTION_X[directionId] +
                                ERROR_PROPAGATION_DIRECTION_Y[directionId] * width;
                        ...
                        uint32_t closestDifference =
                                diffR2 * diffR2 + diffG2 * diffG2 + diffB2 * diffB2;
