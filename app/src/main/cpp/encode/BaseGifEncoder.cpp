...
void
BaseGifEncoder::qsortColorHistogram(uint32_t *imageColorHistogram, int32_t maxColor, uint32_t from,
                                    uint32_t to) {
    if (to == from) {
        return;
    }
...
void BaseGifEncoder::updateColorHistogram(Cube *nextCube, Cube *maxCube, int32_t maxColor,
                                          uint32_t *imageColorHistogram) {
    qsortColorHistogram(imageColorHistogram, maxColor, maxCube->colorHistogramFromIndex,
                        maxCube->colorHistogramToIndex);
    uint32_t median = maxCube->colorHistogramFromIndex +
                      ((maxCube->colorHistogramToIndex - maxCube->colorHistogramFromIndex) >> 1);
    ...
    if (GET_COLOR(imageColorHistogram[nextCube->colorHistogramFromIndex], maxColor) !=
        ...
        if (GET_COLOR(imageColorHistogram[nextCube->colorHistogramFromIndex], maxColor) !=
            GET_COLOR(imageColorHistogram[nextCube->colorHistogramToIndex], maxColor)) {
            if (GET_COLOR(imageColorHistogram[median], maxColor) ==
                GET_COLOR(imageColorHistogram[median + 1], maxColor)) {
                while (GET_COLOR(imageColorHistogram[nextCube->colorHistogramToIndex], maxColor) ==
                       GET_COLOR(imageColorHistogram[median], maxColor)) {
                    --median;
                }
                nextCube->colorHistogramToIndex = median;
            }
    ...
    maxCube->colorHistogramFromIndex = maxCube->colorHistogramToIndex > median + 1 ? median + 1
                                                                                   : maxCube->colorHistogramToIndex;
    nextCube->cMin[maxColor] = GET_COLOR(imageColorHistogram[nextCube->colorHistogramFromIndex],
                                         maxColor);
    nextCube->cMax[maxColor] = GET_COLOR(imageColorHistogram[nextCube->colorHistogramToIndex],
                                         maxColor);
    maxCube->cMin[maxColor] = GET_COLOR(imageColorHistogram[maxCube->colorHistogramFromIndex],
                                        maxColor);
    maxCube->cMax[maxColor] = GET_COLOR(imageColorHistogram[maxCube->colorHistogramToIndex],
                                        maxColor);
}
...
void BaseGifEncoder::computeColorTable(uint32_t *pixels, Cube *cubes, uint32_t pixelNum) {
    ...
    if (0 != frameNum && NULL != lastColorReducedPixels) {
        ...
        memcpy(&colorHistogramMemory[pixelNum], lastColorReducedPixels,
               pixelNum * sizeof(uint32_t));
        pixelNum *= 2;
    } else {
        ...
        for (uint32_t i = 0; i < cubeIndex; ++i) {
            ...
            for (uint32_t colorIdx = 0; colorIdx < COLOR_MAX; ++colorIdx) {
                ...
                if (comparingColor == lastRootColor) {
                    comparingDiff = comparingDiff * 11 /
                                    10; // multiply 110% to reduce color blinking from difference of root color.
                }
    ...
    for (uint32_t i = 0; i < 255; ++i) {
        Cube *cube = &cubes[i];
        for (int32_t color = 0; color < COLOR_MAX; ++color) {
            qsortColorHistogram(colorHistogram, color, cube->colorHistogramFromIndex,
                                cube->colorHistogramToIndex);
            uint32_t median = cube->colorHistogramFromIndex +
                              ((cube->colorHistogramToIndex - cube->colorHistogramFromIndex) >> 1);
            if (median < pixelNum) {
    ...
    for (uint32_t y = 0; y < height; ++y) {
        for (uint32_t x = 0; x < width; ++x) {
            if (0 == (*pixels >> 24)) {
                ...
                *colorReducedPixelOut = (0xFF000000 | (cube->color[BLUE] << 16) |
                                         (cube->color[GREEN] << 8) | cube->color[RED]);
                if (useDither) {
                    diffR = r - (uint32_t) cube->color[RED];
                    diffG = g - (uint32_t) cube->color[GREEN];
                    diffB = b - (uint32_t) cube->color[BLUE];
                    for (int directionId = 0;
                         directionId < ERROR_PROPAGATION_DIRECTION_NUM; ++directionId) {
                        uint32_t *pixel = pixels + ERROR_PROPAGATION_DIRECTION_X[directionId] +
                                          ERROR_PROPAGATION_DIRECTION_Y[directionId] * width;
                        if (x + ERROR_PROPAGATION_DIRECTION_X[directionId] >= width ||
                            y + ERROR_PROPAGATION_DIRECTION_Y[directionId] >= height ||
                            0 == (*pixels >> 24)) {
                            continue;
                        }
                        int32_t weight = ERROR_PROPAGATION_DIRECTION_WEIGHT[directionId];
                        int32_t dstR = ((int32_t) ((*pixel) & 0xFF) + (diffR * weight + 8) / 16);
                        int32_t dstG = (((int32_t) ((*pixel) >> 8) & 0xFF) +
                                        (diffG * weight + 8) / 16);
                        int32_t dstB = (((int32_t) ((*pixel) >> 16) & 0xFF) +
                                        (diffB * weight + 8) / 16);
